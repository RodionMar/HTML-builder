...
// async function removeAssetsFolder() {
//    try{
//       await fsWithPromise.rm(pathToFileCopy, {recursive: true});
//    } catch (err){
//       console.error(err);
//    }
// };

// function removeAssetsFolder() {
//    fs.rm(pathToFileCopy, {recursive: true}, (err) => {
//       if(err) {
//          console.error(err)
//       }
//    });
...
async function moveAssets() {
   try {
      const data = await createCopyAssetsFolder();
      ...
      for await (const folder of readOriginalAssets) {
         ...
         const readCopyAssets = await fsWithPromise.readdir(pathToFileCopy);
